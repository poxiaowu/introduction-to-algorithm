...
bool bfs(int **residual,bool *flag,int index,stack<int> &path ,int n)//广度优先遍历,求图中是否存在从起点到终点的路径
{
	queue<int> que;
	que.push(index);
	int *par=new int[n];//保存父亲结点的数组
	for(int i=0;i<n;++i){
		par[i]=i;
	}
	while(que.size()){
...
void edmonds_karp(int **matrix,int n)//edmonds_karp
{
	stack<int> path;//存放路径的栈
	bool *flag=new bool[n];
	for(int i=0;i<n;++i){
	...
	int min_flow=numeric_limits<int>::max();
	while(bfs(matrix,flag,0,path,n)){		
		for(int i=0;i<n;++i){
			flag[i]=false;
		}
		int size=path.size();
