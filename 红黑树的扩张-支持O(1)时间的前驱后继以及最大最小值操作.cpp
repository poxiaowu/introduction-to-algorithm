...
typedef struct rb_node{//带父节点的二叉树
	...
	rb_node *predecessor,*successor;
}rb_node,*pRBNode;

pRBNode nil;//所有结点的公共黑色结点，类似于NULL

void binary_tree_left_rotation(pRBNode &pn,pRBNode y)//二叉搜索树左旋,旋转时前驱和后继都不再发生变化
{
	pRBNode x=y->right;
	y->right=x->left;
	if(x->left!=nil){
...
void binary_tree_right_rotation(pRBNode &pn,pRBNode x)//二叉搜索树右旋，旋转时前驱和后继都不再发生变化
{
	pRBNode y=x->left;
	x->left=y->right;
	if(y->right!=nil){
...
{	
	...
	if(y==nil){
		pn=pz;
		pn->rb=0;
		pn->predecessor=nil;//初始时，前继为nil，后继续为nil
		pn->successor=nil;
		nil->left=pn;//nil中存放最大最小值
		nil->right=pn;
	}else if(pz->value<=y->value){
		y->left=pz;				
		pz->predecessor=y->predecessor;//插入到左边时，pz的前趋结点为y->predecessor,
		pz->predecessor->successor=pz;
		pz->successor=y;//pz的后继结点为y;
		y->predecessor=pz;//y的前继结点为pz
		if(pz->value<=nil->left->value){//存放最小值,
			nil->left=pz;
		}
	}else{
		y->right=pz;
		pz->predecessor=y;//插入到右子树时，pz的前驱结点为y,后继结点为y->successor
		pz->successor=y->successor;
		pz->successor->predecessor=pz;
		y->successor=pz;//y的后继结点为pz
		if(pz->value>=nil->right->value){//存放最大值,
			nil->right=pz;
		}
	}
	pz->parent=y;
...
int build_binary_search_tree(pRBNode &pn,int n)//建立n个具有父节点的二叉搜索树
{
	...
	for(int i=0;i<n;++i){
		...
		ptmp->rb=1;//红结点
		ptmp->size=1;
		ptmp->predecessor=nil;//前驱
		ptmp->successor=nil;//后继
		cout<<a[i]<<"\t";
...
int main()
{
	...
	nil->predecessor=nil;//前继
	nil->successor=nil;//后继
	pRBNode pn=nil;
	...
	for(int i=0;i<50;++i){
		...
		if(p!=nil){
			cout<<" founded"<<endl;
			cout<<"successor is : "<<binary_tree_successor(p)->value<<"\t"<<p->successor->value<<endl;
			cout<<"predecessor is :"<<binary_tree_predecessor(p)->value<<"\t"<<p->predecessor->value<<endl;
	...
	cout<<"maximum is : "<<p->value<<"\t"<<nil->right->value<<endl;
	p=binary_tree_minimux(pn);
	cout<<"minimum is : "<<p->value<<"\t"<<nil->left->value<<endl;
}     
